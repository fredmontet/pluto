...
| `dew_point` | °C | Dew point temperature (Magnus formula, see below) | derived | `temperature` | `pluto_dew_point_celsius` |
| `absolute_humidity` | g/m³ | Absolute humidity (see below) | derived | — | `pluto_absolute_humidity_grams_per_m3` |
| `aqi` | — | European Air Quality Index band, 1 good .. 6 extremely poor (see below) | derived | `aqi` | `pluto_european_aqi` |

The `mock` driver mimics the sensor metrics with the same names and
...
Each metric can be calibrated per device with a
`[sensors.<driver>.<metric>]` table in `pluto.toml`, applied in this
order:

1. `cpu_temp_compensation` — the BME280 temperature correction
   `value − (cpu_temp − value) / factor`, where `cpu_temp` is the
   Pi's SoC temperature smoothed over the last 5 samples. On by
   default for `temperature` with factor 2.25 (higher = weaker,
   `0` disables); `raw_temperature` is never compensated.
2. `scale`, then `offset` — two-point calibration
   (`value × scale + offset`).
3. `smooth` — moving average over the last N samples. While the
   window is still filling, the average of the samples seen so far is
   reported.

## Derived metrics

Enabled individually in the `[derived]` section and computed from the
*transformed* values:

- **Dew point** (°C) — Magnus formula with Sonntag constants
  b = 17.62, c = 243.12 °C:
  γ = ln(RH/100) + b·T/(c+T), then T_d = c·γ/(b−γ).
- **Absolute humidity** (g/m³) —
  AH = 216.7 · (RH/100 · 6.112 · e^(b·T/(c+T))) / (273.15 + T).
- **Air quality index** — the [European Air Quality Index](https://airindex.eea.europa.eu/)
  band for the current PM2.5/PM10 readings; the overall index is the
  worst band among the available pollutants:

  | Band | Meaning | PM2.5 (µg/m³) | PM10 (µg/m³) |
  |---|---|---|---|
  | 1 | Good | 0–10 | 0–20 |
  | 2 | Fair | 10–20 | 20–40 |
  | 3 | Moderate | 20–25 | 40–50 |
  | 4 | Poor | 25–50 | 50–100 |
  | 5 | Very poor | 50–75 | 100–150 |
  | 6 | Extremely poor | > 75 | > 150 |
